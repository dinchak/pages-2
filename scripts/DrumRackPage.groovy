...
class DrumRackPage extends GroovyAPI {
    ...
    void stop() {
        log("DrumRackPage shutting down")
        patterns().clearIgnore()
    }

    void recordedPress(int x, int y, int val, int pattNum) {
        int note = ((y * sizeX()) + x + (song * 48)) % 128
        int channel = ((y * sizeX()) + x + (song * 48)) / 128
        if (val == 1) {
